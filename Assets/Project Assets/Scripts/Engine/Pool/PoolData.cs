...
public class PoolData:MonoBehaviour
{
	...
	public void Initialize(string aPath, string aPrefab, int anAmount){
		...
		//gameObject.name = aPrefab+"_Pool"; // new gameobject
		//holder.transform.ResetToParent(gameObject); // parent this holder to the PoolManager holder
		...
		source.transform.ResetToParent(gameObject);
		freeObjects.Add(source);
		for (int i = 0; i < anAmount-1; i++) { // create the amount of objects
			CreateObject(source);
		}
        gameObject.SetActive(false); // Hide the holder so we dont have all those objects floating in the game
	}
	...
	public void ReturnObject(GameObject anObject){
		...
		//}
		// set back
		anObject.transform.ResetToParent(gameObject);
